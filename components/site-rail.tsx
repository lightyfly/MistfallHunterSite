  ...
  return (
    <aside className="site-rail" aria-label={copy.rail.navigation}>
      <div className="rail-nav-card">
        ...
        {sections.map((section) => <details className="rail-section" open={section.key === "classes" && classOpen} key={section.key}><summary><span className="rail-icon">{section.key === "classes" ? "◈" : section.key === "bosses" ? "✣" : section.key === "updates" ? "▦" : "◌"}</span><span>{section.label}</span><span className="rail-count">{section.count}</span></summary>{section.key === "classes" ? <div className="rail-subnav"><Link href={localizedPath(locale, "/classes")}>{copy.rail.overview}</Link><Link href={localizedPath(locale, "/classes/overview")}>{copy.rail.detail}</Link><Link href={localizedPath(locale, "/classes/builds")}>{copy.nav.classes} &amp; Builds</Link></div> : null}</details>)}
